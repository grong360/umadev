...
## 附录:CC 全功能差距盘点(2026-06-28 三代理审计)

> 结论:CC 有 ~90 命令 + ~45 工具,但**绝大多数是底座的活**(UmaDev 借底座,底座自带)**或对"总监壳"无关**。真正该补的 **~13 项**,不追 CC 全表(克隆=降级)。
> UmaDev 在 9 个维度**已领先 CC**(别回退):RAG 知识库 · repo-map 注入 · 自进化记忆(踩坑→反思→效力闭环)· 前后端契约校验 · 运行时证明 · 可验收 Plan DAG · 结构化评审团 · 治理+合规 · 三底座可移植。

### A. 会话续接(最高优先)
- **/resume 聊天续接** —— 聊天路径只存 ≤16 条转录、**没存 base_session_id**;`/resume` 把文件 id 当底座 session id(对不上)→ 冷启动丢上下文。照搬 `/run` 已修好的 base_session_id+`--resume` 方案(S–M)。

### B. 能力缺口(~5)
- **后台/异步运行** + 任务注册表 + TUI 管理(HIGH, L)—— 现在团队全在一个前台 run 同步跑。
- PostToolUse 治理钩子实际安装(文档说装了,实际只装了 PreToolUse)(MED, S)
- 用户自定义角色 `.umadev/agents/*.md`(现 8 个写死)(MED, M)
- MCP server 暴露 plan/verify/contract/lessons(现仅 2 个治理工具)(MED-LOW, M)
- 自由提问 AskUserQuestion(LOW, S)

### C. UX 成熟度(~7,"显得成熟")
- **P0**:`@` 文件引用(M,已有 repo-map 喂候选)· **Shift+Tab 三态循环**(S,现仅 2 态够不着 Plan)· **思考过程显示**(M-L,reasoning 在驱动层被丢)
- **P1**:转录内搜索(M)· 双击 Esc 回退重发某条(M-L)· 上下文/花费仪表(M)· 完成通知 响铃/桌面(S-M)
- **P2**:反向搜索 Ctrl+R · 图片字节粘贴 · 主题选择器 · `!`-bash · 快捷键速查 · `/config` 面板 · 连续读折叠组 · vim/自定义键位(长尾)

### 推进顺序
idle 活性判断 → **/resume** → Shift+Tab 三态 → `@` 引用 → 思考显示 → 后台运行 → 搜索/通知 …… 每个提交+测试,攒批发版。
